### ALS_R1R2 Cross Validation
### Author: Mengying Shi

ALS_R1R2.cv <- function(dat_train, K, f, lambda){
  ### Input:
  ### - train data frame
  ...
  for (i in 1:K){
    ...
    result <- ALS_R1R2(f = f, lambda =lambda, max.iter = 20, data = dat_train, train = train.data, test = test.data)
